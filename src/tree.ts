 ...
 */
function simplifyArrayOfTrees(
  parse: OriginalParseTree[],
): SimplifiedParseTree[] {
  let result: SimplifiedParseTree[] = [];
...
function simplifyFunction(type: string) {
  return (parse: OriginalParseTree[]) => ({
    type:
      type
      + (parse.length === 2
      && typeof parse[1][0] === 'string'
      && /[A-Z]+/.test(parse[1][0])
        ? '（非表示）'
        : ''),
...
function simplifyFunctionUnconditionalShow(type: string) {
  return (parse: OriginalParseTree[]) => ({
    type,
    children: simplifyArrayOfTrees(parse.slice(1)),
  });
...
const simplifyFunctions = {
  ...
  bridi_tail_1: (parse: OriginalParseTree) => ({
    type: `ブリディ末端第一下位（bridi tail 1）${
      (parse.length === 2
        && parse[0][0] === 'bridi_tail_2'
        && parse[1][0] === 'bridi_tail_3')
      || (parse.length === 3
        && parse[1][0] === 'bridi_tail_2'
        && parse[2][0] === 'bridi_tail_3')
        ? '（非表示）'
        : ''
    }`,
  ...
  bridi_tail_3: (parse: OriginalParseTree) => ({
    type: `ブリディ末端第三下位（bridi tail 3）${
      parse.length === 3
      && parse[2].length === 2
      && parse[2][1].length === 1
      && parse[2][1][0] === 'VAU'
  ...
  terms_gik_terms: simplifyFunction(
    '項コレクション-gik-項コレクション（terms_gik_terms）',
  ),
  nonabs_terms: simplifyFunction('非吸収性項コレクション（nonabs terms）'),
  nonabs_terms_1: simplifyFunction(
    '非吸収性項コレクション第一下位（nonabs terms 1）',
  ),
  nonabs_terms_2: simplifyFunction(
    '非吸収性項コレクション第二下位（nonabs terms 2）',
  ),
  ...
  sumti_6: (parse: OriginalParseTree) => {
...
export const simplifyTree = (
  ...
  if (typeof type === 'string' && type in simplifyFunctions) {
    return [
      simplifyFunctions[<keyof typeof simplifyFunctions>type](
        parse as OriginalParseTree[],
      ),
    ];
  }
  ...
  if (typeof type === 'string') {
    result = simplifyArrayOfTrees(parse.slice(1) as OriginalParseTree[]);
  } else {
    result = simplifyArrayOfTrees(parse as OriginalParseTree[]);
  }
 ...
 */
export const numberSumti = (parse: SimplifiedParseTree) => {
...
const placeTagToPlace = (tag: SimplifiedParseTreeWithWord) => {
  if (tag.word === 'fa') {
    return 1;
  }
  if (tag.word === 'fe') {
...
const bridiTailRecursion = (
  child: SimplifiedParseTreeWithChildren,
  baseCounter: number | 'fai',
) => {
  // now walk through this array
  let nextIsModal = false;
  let sumtiCounter = baseCounter;
  for (let j = 0; j < child.children.length; j += 1) {
    const subchild = child.children[j];
    if (
      /FA|制詞（tense modal）|BAI|FIhO|PU|スムティ（sumti）|セルブリ（selbri）/g.test(
        subchild.type,
      )
    ) {
      if ('word' in subchild && subchild.type === 'FA') {
        sumtiCounter = placeTagToPlace(subchild);
      }

      if (
        subchild.type === '制詞（tense modal）'
        || subchild.type === 'BAI'
        || subchild.type === 'FIhO'
        || subchild.type === 'PU'
      ) {
    ...
    } else if ('children' in subchild && /ブリディ末端/g.test(subchild.type)) {
      bridiTailRecursion(subchild, baseCounter);
    } else if (
      'children' in subchild
      && /ブリディ末端|項/.test(subchild.type)
    ) {
...
const numberSumtiInSentence = (parse: SimplifiedParseTreeWithChildren) => {
  let baseCounter: number | 'fai' = 1;
